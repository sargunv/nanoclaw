...
- **Run `mise install` liberally** when tools or dependencies are missing in a mise-managed project. Missing binaries, outdated tool versions, or failed postinstall hooks (browser installs, `pnpm install`, `go mod download`, etc.) are all fixed by re-running `mise install`. It's idempotent and safe.
- **In non-mise-managed projects** (e.g. third-party repos you're contributing to), set up a `mise.local.toml` (git-excluded by default) to declare the tools needed without touching tracked files. Create it manually or via `mise use --path mise.local.toml <tool>@<version>`.
